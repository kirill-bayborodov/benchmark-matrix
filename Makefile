...
BENCH_CLI_SRC ?= $(SRC_DIR)/benchmark_matrix_cli.c
BENCH_CLI_OBJ ?= $(BUILD_DIR)/benchmark_matrix_cli.o
BENCH_CLI_BIN ?= $(BIN_DIR)/bench_matrix

# --- Генерация тестовых фикстур ---
FIXTURES_DIR = $(BUILD_DIR)/fixtures
MOCK_MANIFEST = $(FIXTURES_DIR)/mock_manifest.json
MOCK_BIN = $(FIXTURES_DIR)/mock_bin.sh
...
RUNNER_OPT = --iterations 101 --warmup 5 --data-count 16 --input-kind nonzero --operation-kind xor --measure-mode kernel-only --size-profile medium --capacity-profile normal
...
test: $(MOCK_MANIFEST) $(MOCK_BIN) $(BENCH_CLI_BIN) $(ALL_TEST_BINS)
	...
	for t in $(ALL_TEST_BINS); do \
	  total=$$((total+1)); \
	  echo "--- $$t ---"; \
	  if ./$$t $(RUNNER_OPT); then :; else fail=$$((fail+1)); echo "*** $$t FAILED ***"; fi; \
	done; \
...
$(BENCH_CLI_OBJ): $(BENCH_CLI_SRC)
	@echo "Builds the CLI object file '$@'..."
	$(CC) $(CFLAGS) -c $< -o $@

$(BENCH_CLI_BIN): $(OBJ) $(BENCH_CLI_OBJ)
	@echo "Linking final executable '$@'..."
	@mkdir -p bin
	$(CC) $(CFLAGS) $^ -o $@ -Llibs/json-lib/dist -ljson_lib

$(FIXTURES_DIR):
	@mkdir -p $@

$(MOCK_MANIFEST): | $(FIXTURES_DIR)
	@echo "Generating mock manifest..."
	@echo '{"schema_version": 1, "profiles": [{"id": "p1", "input_kind": "a", "operation_kind": "b", "measure_mode": "c", "size_profile": "d", "capacity_profile": "e"}]}' > $@

$(MOCK_BIN): | $(FIXTURES_DIR)
	@echo "Generating mock binary..."
	@echo '#!/bin/sh' > $@
	@echo 'echo "benchmark=test elapsed_seconds=0.1 ns_per_call=10.0"' >> $@
	@echo 'echo "Benchmark finished."' >> $@
	@echo 'exit 0' >> $@
	@chmod +x $@
